...
{
	static uint64_t Offset, Length;
	u8 STA; 
	if (TransferState == TXFR_IDLE )
	{
		Offset = (uint64_t)Memory_Offset * Mass_Block_Size[lun];
		Length = (uint64_t)Transfer_Length * Mass_Block_Size[lun];
...
{ 
	static uint64_t W_Offset, W_Length;
	u8 STA;   
	...
	if (TransferState == TXFR_IDLE )
	{
		...
		W_Length = (uint64_t)Transfer_Length * Mass_Block_Size[lun];
		TransferState = TXFR_ONGOING;
	} 
	if (TransferState == TXFR_ONGOING )
	{
