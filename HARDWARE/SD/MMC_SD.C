...
{
 	SPI2_SetSpeed(SPI_SPEED_2);//设置到高速模式	
}
//SPI硬件层初始化
void SD_SPI_Init(void)
{
	...
	RCC->APB2ENR|=1<<6;     //PORTA时钟使能 
	GPIOE->CRL&=0XFFFF0FFF; 
	GPIOE->CRL|=0X00003000;	//PA2.3.4 推挽 	    
	GPIOE->ODR|=0X7<<2;    	//PA2.3.4上拉 
	SPI2_Init();
	SD_CS=1;
}
///////////////////////////////////////////////////////////////////////////////////
...
//返回值:1,sd卡在位
//      0,sd卡不在位
u8 SD_GetStatus(void)
{
    u8 r1;
    //发CMD13命令，读卡状态寄存器
    r1=SD_SendCmd(CMD13,0,0x01); 
	SD_DisSelect();//取消片选
	if(r1)return 0;
	else return 1;
} 
