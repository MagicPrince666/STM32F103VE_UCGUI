...
#define SAVE_ADDR_BASE 0X0807F800
//保存校准参数										    
...
//    AT24CXX_WriteLenByte(SAVE_ADDR_BASE+4,temp,4);
	//保存x偏移量
	for(i=0;i<2;i++)
	   data[8+i]=tp_dev.xoff>>(8*i);
//    AT24CXX_WriteLenByte(SAVE_ADDR_BASE+8,tp_dev.xoff,2);		    
	//保存y偏移量
	for(i=0;i<2;i++)
	   data[10+i]=tp_dev.yoff>>(8*i);
...
//	AT24CXX_WriteOneByte(SAVE_ADDR_BASE+13,temp); 
	...
	STMFLASH_Write(SAVE_ADDR_BASE,(u16 *)data,14);
...
{					  
	s32 tempfac;	
	u8 data[14];
	STMFLASH_Read(SAVE_ADDR_BASE,(u16 *)data,14);//读取标记字,看是否校准过！ 
	tempfac=data[13];		 
	if(tempfac==0X0A)//触摸屏已经校准过了			   
	{    												 
		tempfac=(data[0]<<0)|(data[1]<<8)|(data[2]<<16)|(data[3]<<24);
		//AT24CXX_ReadLenByte(SAVE_ADDR_BASE,4);		   
		tp_dev.xfac=(float)tempfac/100000000;//得到x校准参数
		
		tempfac=(data[4]<<0)|(data[5]<<8)|(data[6]<<16)|(data[7]<<24);
		//AT24CXX_ReadLenByte(SAVE_ADDR_BASE+4,4);			          
		tp_dev.yfac=(float)tempfac/100000000;//得到y校准参数
	    //得到x偏移量
		tp_dev.xoff=(data[8]<<0)|(data[9]<<8);
		//AT24CXX_ReadLenByte(SAVE_ADDR_BASE+8,2);			   	  
 	    //得到y偏移量
		tp_dev.yoff=(data[10]<<0)|(data[11]<<8);
		//AT24CXX_ReadLenByte(SAVE_ADDR_BASE+10,2);				 	  
 		tp_dev.touchtype=data[12];
		//AT24CXX_ReadOneByte(SAVE_ADDR_BASE+12);//读取触屏类型标记
...
{	
		...
		RCC->APB2ENR|=1<<2|1<<3|1<<5|1<<6;    //PB时钟使能	     
#ifndef SPI_Control	
		GPIOB->CRH&=0X0000FFFF;//PB1  2
		GPIOB->CRH|=0X38330000; 
	  GPIOB->ODR|=0xf<<12;      //PB1 2 推挽输出 
#else	
    GPIOB->CRH&=0XFFF0FFFF;//PB1  2
	  ...
	  GPIOD->CRH|=0X00300000;	//PD13 推挽 	    
	  GPIOD->ODR|=1<<13;    	//PD13上拉
	
	  GPIOE->CRL&=0XFFFF0FFF; 
	  GPIOE->CRL|=0X00003000;	//PE3 推挽 	    
	  GPIOE->ODR|=1<<3;    	//PE3上拉
		GPIOA->CRH&=0XFFFFFFF0;
		GPIOA->CRH|=0X00000008;
		GPIOA->ODR|=1<<8;      //PF8,9,10 全部上拉	  
	  SPI2_Init();
